(function (exports, Laya) {
	...
	class RecInfo {
	    ...
	    static getGlobalPoints(sprite, x, y) {
	        return sprite.posGlob(new Laya.Point(x, y));
	    }
	...
	class Axis extends Laya.Sprite {
	    ...
	    controlMouseDown(e) {
	        ...
	        this.oPoint = this.posGlob(this.oPoint);
	...
	class NodeInfosItem extends Laya.Sprite {
	    ...
	    static getNodeValue(node, key) {
	        ...
	        switch (key) {
	            case "x":
	                rst = node["x"] + " (g:" + node.posGlob(NodeInfosItem._nodePoint).x + ")";
	                break;
	            case "y":
	                rst = node["y"] + " (g:" + node.posGlob(NodeInfosItem._nodePoint).y + ")";
	                break;
	            default:
	                rst = node[key];
	        }
	    ...
	    showInfo(node) {
	        ...
	        WalkTools.walkArr(pointList, node.posGlob, node);
	...
	class DebugTool {
	    ...
	    static showDisBound(sprite = null, clearPre = true, color = "#ff0000") {
	        ...
	        if (!pointList || pointList.length < 1)
	            return;
	        pointList = Laya.GrahamScan.pListToPointList(pointList, true);
	        WalkTools.walkArr(pointList, sprite.posGlob, sprite);
	        pointList = Laya.GrahamScan.pointListToPlist(pointList);
	...
	class DragBox extends Laya.Sprite {
	    ...
	    changePoint() {
	        var p = this._target.posGlob(new Laya.Point());
